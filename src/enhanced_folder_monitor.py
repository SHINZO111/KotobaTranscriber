...
class AsyncFolderMonitor(QObject):
    ...
    def __init__(self, folder_paths: Optional[List[str]] = None, check_interval: float = 1.0, parent=None):
        """
        初期化
    ...
    def _on_file_event(self, file_path: str, event_type: str):
        ...
        with self._lock:
            ...
            self._pending_files[path] = FileEvent(path=file_path, event_type=event_type, timestamp=time.time())
        ...
        for path in folder.iterdir():
            if path.is_file() and path.suffix.lower() in self.AUDIO_EXTENSIONS:
                ...
                if abs_path not in self._processed_files:
                    with self._lock:
                        self._pending_files[path] = FileEvent(path=str(path), event_type="existing", timestamp=time.time())
    ...
    def _save_processed_files(self):
        ...
        try:
            ...
            data = {"files": list(self._processed_files), "updated": datetime.now().isoformat()}
            content = json.dumps(data, ensure_ascii=False, indent=2)
            fd, tmp_path = tempfile.mkstemp(dir=str(self._processed_list_path.parent), suffix=".tmp")
            try:
