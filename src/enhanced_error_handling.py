...
class ErrorHandler:
    ...
    def __init__(self, max_history: int = 100):
        ...
        self._handlers: Dict[ErrorSeverity, List[Callable[[ErrorRecord], None]]] = {severity: [] for severity in ErrorSeverity}
        self._recovery_strategies: Dict[str, Callable[[Exception], bool]] = {}
        self._consecutive_errors = 0
        self._max_consecutive_errors = 5
        self._handle_lock = threading.Lock()

    def register_handler(self, severity: ErrorSeverity, handler: Callable[[ErrorRecord], None]):
        """エラーハンドラーを登録"""
        with self._handle_lock:
            self._handlers[severity].append(handler)

    def register_recovery_strategy(self, error_type: Type[Exception], strategy: Callable[[Exception], bool]):
        """回復戦略を登録"""
        with self._handle_lock:
            self._recovery_strategies[error_type.__name__] = strategy

    def handle(self, error: Exception, severity: ErrorSeverity = ErrorSeverity.ERROR, context: Dict[str, Any] = None) -> bool:
        """
        エラーを処理
...
def safe_execute(
    func: Callable, default_return: Any = None, error_message: str = None, severity: ErrorSeverity = ErrorSeverity.ERROR
):
    """
    安全に関数を実行
...
def retry_on_error(max_retries: int = 3, delay: float = 1.0, exceptions: Tuple[Type[Exception], ...] = (Exception,)):
    """
    デコレータ: エラー時にリトライ
...
def with_error_handling(error_message: str = None, severity: ErrorSeverity = ErrorSeverity.ERROR, reraise: bool = False):
    """
    デコレータ: エラーハンドリングを追加
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                handler = get_error_handler()
                _msg = error_message or str(e)  # noqa: F841
                context = {
...
class FileOperationGuard:
    ...
    @staticmethod
    def safe_read(file_path: str, encoding: str = "utf-8", default: str = None) -> Optional[str]:
        """安全にファイルを読み込み"""
        try:
    ...
    @staticmethod
    def safe_delete(file_path: str, move_to_trash: bool = False) -> bool:
        """安全にファイルを削除"""
        try:
