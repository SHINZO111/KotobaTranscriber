...
class ConstructionVocabulary:
    ...
    def save_vocabulary(self):
        """語彙ファイルを保存"""
        try:
            data = {"hotwords": self.hotwords, "replacements": self.replacements, "categories": self.category_vocabularies}

            self.vocabulary_file.parent.mkdir(parents=True, exist_ok=True)
            import tempfile

            fd, tmp_path = tempfile.mkstemp(dir=str(self.vocabulary_file.parent), suffix=".tmp")
            try:
    ...
    def create_default_vocabulary(self):
        ...
        # すべての用語をマージ
        all_terms = (
            self.STANDARD_LABOR_TERMS + self.CONSTRUCTION_LAW_TERMS + self.COST_MANAGEMENT_TERMS + self.AGEC_SPECIFIC_TERMS
        )
