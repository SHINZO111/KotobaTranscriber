...
class ErrorRecoveryManager:
    ...
    def __init__(self, log_dir: Optional[str] = None):
        ...
        """
        if log_dir is None:
            log_dir = str(Path.home() / ".kotoba_transcriber" / "error_logs")
        ...
        self._error_count = 0
        self._lock = Lock()
        self._error_callbacks: Dict[str, List[Callable]] = {"transient": [], "resource": [], "permanent": []}
    ...
    def handle_error(
        ...
        if error_category == "transient" and retry_func:
            ...
            try:
                ...
                return {"success": True, "action": RecoveryAction.FALLBACK.value, "result": result, "error": None}
        ...
        self._log_error(error_record)
        return {"success": False, "action": RecoveryAction.SKIP.value, "result": None, "error": str(error)}
    ...
    def _classify_error(self, error: Exception) -> str:
        ...
        # 一時的エラー
        transient_errors = ["TimeoutError", "ConnectionError", "TemporaryFailure", "ChunkedEncodingError", "ReadTimeout"]
        transient_keywords = ["timeout", "temporary", "retry", "connection reset"]

        # リソースエラー
        resource_errors = ["MemoryError", "OSError", "IOError", "FileNotFoundError", "PermissionError", "DiskFullError"]
    ...
    def _retry_with_backoff(self, func: Callable, error_record: ErrorRecord, max_retries: int = 3) -> Dict[str, Any]:
        """
        指数バックオフでリトライ
    ...
    def get_error_summary(self, since_hours: Optional[int] = None) -> Dict[str, Any]:  # noqa: C901
        """
        エラーサマリーを取得
        ...
        recovery_stats = {"retry": 0, "fallback": 0, "skip": 0, "recovered": 0, "retry_success": 0, "retry_failed": 0}
        ...
        return {
            ...
            "most_problematic_files": sorted(file_errors.items(), key=lambda x: x[1], reverse=True)[:5],
        }
...
class Resilient:
    ...
    def __init__(self, max_retries: int = 3, fallback_value: Any = None, log_dir: Optional[str] = None):
    ...
    def __call__(self, func: Callable) -> Callable:
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                return self.manager.handle_error(e, str(file_path), retry_fn, fallback_fn, self.max_retries)
...
if __name__ == "__main__":
    ...
    attempt_count = 0
    result = manager.handle_error(ConnectionError("Test error"), "test_file.wav", retry_func=flaky_function, max_retries=3)
    print(f"   Result: {result}")

    # フォールバックテスト
    print("\n2. Fallback Test:")
    result = manager.handle_error(MemoryError("Out of memory"), "large_file.wav", fallback_func=fallback_function)
    print(f"   Result: {result}")
