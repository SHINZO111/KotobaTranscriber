...
class JSONFormatter:
    ...
    def format(self, segments: List[Dict], metadata: Optional[Dict] = None) -> str:
        """
        JSON形式で出力
...
class TXTFormatter:
    ...
    def format(self, segments: List[Dict], include_timestamps: bool = True, include_speakers: bool = True) -> str:
        """
        プレーンテキストで出力
...
class DOCXFormatter:
    ...
    def format(
        self, segments: List[Dict], output_path: str, title: str = "文字起こし結果", include_speakers: bool = True
    ) -> bool:
        """
        Wordドキュメントで出力
...
class EnhancedSubtitleExporter:
    ...
    def export(self, segments: List[Dict], output_path: str, format_type: str, options: Optional[Dict] = None) -> bool:
        """
        エクスポート実行
        ...
        try:
            ...
            if format_type in self.FORMATTERS:
                ...
                content = self.json_formatter.format(segments, options.get("metadata"))
    ...
    def _export_subtitle(self, segments: List[Dict], output_path: str, format_type: str) -> bool:
        """字幕ファイルをエクスポート"""
    ...
    def export_auto(
        self, segments: List[Dict], base_path: str, formats: List[str] = None, options: Optional[Dict] = None
    ) -> Dict[str, bool]:
        """
        複数フォーマットで自動エクスポート
        ...
        for fmt in formats:
            ...
            results[fmt] = self.export(segments, str(output_path), fmt, options)

        return results

    def merge_short_segments(self, segments: List[Dict], min_duration: float = 1.0, max_chars: int = 40) -> List[Dict]:
        """短いセグメントをマージ"""
        return list(_merge_short_segments(segments, min_duration, max_chars))

    def split_long_segments(self, segments: List[Dict], max_chars: int = 40, max_duration: float = 5.0) -> List[Dict]:
        """長いセグメントを分割"""
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        results = exporter.export_auto(test_segments, base_path, formats=["srt", "vtt", "json", "txt"])
