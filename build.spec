...
# 統合アプリ用のフィルタリング
unified.datas = [x for x in unified.datas if not any(pattern in x[0] for pattern in excluded_patterns)]
unified.binaries = [x for x in unified.binaries if not any(pattern in x[0] for pattern in excluded_patterns)]

pyz = PYZ(a.pure, a.zipped_data, cipher=block_cipher)
unified_pyz = PYZ(unified.pure, unified.zipped_data, cipher=block_cipher)
...
unified_exe = EXE(
    ...
    [],
    exclude_binaries=True,
    name='KotobaTranscriber_Unified',
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=True,
