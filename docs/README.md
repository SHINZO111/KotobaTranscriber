...
### 設定ファイル
アプリケーション設定は `app_settings.json` に自動保存されます：
- ウィンドウ位置・サイズ
- 監視フォルダ設定
- テキスト整形オプション
- AI補正設定
...
- **自動クリーンアップ**: 古いバックアップは自動削除

### バックアップ復元
設定ファイルが破損した場合、手動で復元可能：
1. `.backups/` フォルダを開く
2. 復元したいバックアップをコピー
3. `app_settings.json` に上書き
...
### 設定ファイルが破損した
1. `.backups/` フォルダから最新のバックアップを確認
2. バックアップを `app_settings.json` に復元
3. それでも解決しない場合、ファイルを削除してデフォルト設定で再起動

### フォルダ監視が停止する
- 処理中のファイルが残っている可能性があります
- アプリケーションを再起動すると自動的にクリーンアップされます
- TTL（1時間）経過後、自動的に処理再開されます
...
KotobaTranscriberは完全に**無償で商用利用可能**です：
- ✅ 個人利用・商用利用ともに自由
- ✅ ソースコード開示不要
- ✅ 改変・再配布可能
...
このアプリケーションは以下のオープンソースライブラリを使用しています。すべて商用利用可能です：

- **PySide6** (LGPL v3) - GUIフレームワーク
- **Kotoba-Whisper** (Apache 2.0) - 日本語音声認識モデル
- **PyTorch** (BSD-3-Clause) - 深層学習フレームワーク
- **Transformers** (Apache 2.0) - 事前学習モデル
- **その他** - 詳細は[THIRD_PARTY_LICENSES.md](../THIRD_PARTY_LICENSES.md)を参照

詳細なライセンス情報は、プロジェクトルートの`LICENSE`および`THIRD_PARTY_LICENSES.md`をご覧ください。
