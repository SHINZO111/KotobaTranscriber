...
class TestErrorRecoveryManager:
    ...
    def test_init_creates_log_dir(self, tmp_path):
        ...
        _mgr = ErrorRecoveryManager(log_dir=str(log_dir))  # noqa: F841
    ...
    def test_handle_error_fallback(self, tmp_path):
        ...
        mgr = ErrorRecoveryManager(log_dir=str(tmp_path))
        result = mgr.handle_error(MemoryError("oom"), "test.wav", fallback_func=lambda: "fallback_result")
    ...
    def test_handle_error_retry_success(self, tmp_path):
        ...
        result = mgr.handle_error(ConnectionError("initial"), "test.wav", retry_func=flaky, max_retries=3)
        assert result["success"] is True
    ...
    def test_handle_error_retry_all_fail(self, tmp_path):
        ...
        result = mgr.handle_error(
            ConnectionError("initial"), "test.wav", retry_func=always_fail, max_retries=1  # 1回だけリトライ（高速テスト）
        )
        assert result["success"] is False
...
class TestErrorRecord:
    ...
    def test_default_values(self):
        ...
        record = ErrorRecord(
            timestamp="2024-01-01T00:00:00", file_path="test.wav", error_type="ValueError", error_message="test"
        )
    ...
    def test_to_dict(self):
        ...
        record = ErrorRecord(timestamp="2024-01-01", file_path="test.wav", error_type="ValueError", error_message="msg")
        d = asdict(record)
...
@pytest.mark.skipif(not _HAS_SCHEMAS, reason="pydantic not available")
